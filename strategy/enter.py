...
def check_breakthrough(code_name, data, end_date=None, threshold=30):
    ...
    if (
        last_close > max_price > second_last_close
        and max_price > last_open
...
def check_ma(code_name, data, end_date=None, ma_days=250):
    ...
    data[ma_tag] = pd.Series(
        tl.MA(data["收盘"].values, ma_days), index=data.index.values
    )
...
def check_continuous_volume(
    ...
    msg = "*{0} 量比：{1:.2f}\n\t收盘价：{2}\n".format(
        code_name, last_vol / mean_vol, last_close
    )
    logging.debug(msg)
    return True
